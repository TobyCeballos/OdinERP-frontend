...
const Sales = () => {
  ...
  const handlePayOff = async (sellId) => {
    try {
      ...
      const response = await axios
        .put(`${API_ENDPOINT}api/sells/payoff/${sellId}`, {}, { headers })
        .then((res) => {
          fetchSales();
        });
    } catch (error) {
  ...
  const handleSearch = async (event) => {
    ...
    if (value.trim() === "") {
      ...
      try {
        const response = await axios.get(`${API_ENDPOINT}api/sells?page=${currentPage}`, {
          headers,
        });
        setSales(response.data);
      } catch (error) {
  ...
  const handleRowClick = (productId) => {
    ...
    navigate(`/POS/sales/details/${productId}`);
  };
  return (
